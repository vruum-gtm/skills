...
**Cold drafts to people who already replied (VRU-894).** An opener or follow-up item that carries `prior_reply` is a re-pitch to someone who wrote back. Never bulk-approve it. Show the user the reply excerpt and intent, and hold the draft unless they explicitly want the cold sequence to continue. Never call `manage_outreach` action=resume to release a stuck reply: replies send without the plan, and resume re-arms the cold sequence. A resume of a reply-paused plan returns 409 `plan_paused_by_reply` with the pending cold messages it would release; pass `payload: {continue_sequence: true}` only on the user's explicit decision. To end outreach to that person, use action=stop.
